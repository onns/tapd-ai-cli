...
func init() {
	...
	tcaseListCmd.Flags().StringArrayVar(&flagFilter, "filter", nil, "高级过滤条件（可重复，格式：field=OP<value>，支持 LIKE/EQ/CONTAINS 等 OpenAPI 特殊查询语法）")
...
func runTCaseList(cmd *cobra.Command, args []string) error {
	...
	tcases, err := listWithFilters[model.TCase](context.Background(), apiClient, "/tcases", req.ToParams(), flagFilter, "Tcase")
	if err != nil {
